...
--- @return nil
M.command = function()
	local cmd = M.last_cmd or ""

	if cmd == nil or cmd == "" then
		cmd = vim.fn.input("Compile command: ", M.last_cmd or "")
	end
	...
	if M.last_cwd == nil then
		cwd = vim.fn.input("CWD: ", cwd)
	end

	M.executor(cmd, cwd)
end

M.editCmdAndCwd = function()
	M.last_cmd = vim.fn.input("New Cmd:", "")
	M.last_cwd = vim.fn.input("New Cwd:", vim.fn.getcwd())
end
